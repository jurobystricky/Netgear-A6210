...
*/
	...
	if (IS_USB_INF(ad)) {
		...
		if (ret != 0) {
			...
			return; //jb removed STATUS_UNSUCCESSFUL;
		}
	}
#endif /* RTMP_MAC_USB */
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s(): Switch to Ch#%d(%dT%dR), BBP_BW=%d\n",
			__FUNCTION__, channel, 2, 2, bw));
}
...
static BOOLEAN mt76x2_ate_tx_pwr_Evaluation(RTMP_ADAPTER *ad)
{
	PATE_INFO pATEInfo = &(ad->ate);
...
VOID mt76x2_ate_rx_vga_init(PRTMP_ADAPTER ad)
{
...
VOID mt76x2_ate_set_tx_rx(PRTMP_ADAPTER ad, CHAR tx, CHAR rx)
{
	PATE_INFO   pATEInfo = &(ad->ate);
	u32 BbpValue = 0;
...
VOID mt76x2_ate_set_tx_rx_path(PRTMP_ADAPTER ad)
{
	PATE_INFO pATEInfo = &(ad->ate);
...
//JB: Morons, what is the proper return value here?
static BOOLEAN mt76x2_set_ate_tx_bw_proc(PRTMP_ADAPTER ad, PSTRING arg)
{
	u32 core, core_r1 = 0, core_r4 = 0;
	u32 agc, agc_r0 = 0;
	u32 ret;
	u8 BBPCurrentBW = simple_strtol(arg, 0, 10);
	...
	if ((BBPCurrentBW == 0)) {
		...
		//JB: Morons, check warnings. BW_10 does not fit into 2 bits!
		//jb removed ad->ate.TxWI.TXWI_N.BW = BW_10;
		DBGPRINT(RT_DEBUG_ERROR, ("Set_ATE_TX_BW_Proc!! Error!! BW_10\n"));
//jb ???		return TRUE;
		return FALSE;
 	} else {
