...
static VOID CFG80211_RemainOnChannelInit(RTMP_ADAPTER *pAd)
{
...
static VOID CFG80211RemainOnChannelTimeout(
...
#endif /*RT_CFG80211_P2P_CONCURRENT_DEVICE */
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_ROC: RemainOnChannelTimeout -- FINISH\n"));
			...
			cfg80211_remain_on_channel_expired( pWdev,
				...
				//jb was pCfg80211_ctrl->Cfg80211ChanInfo.ChanType, GFP_KERNEL);
				 GFP_KERNEL);
		...
		if (pEntry && pEntry->CFGP2pInfo.NoADesc[0].bValid == TRUE) {
			...
			if (pEntry->CFGP2pInfo.NoADesc[0].bInAwake == TRUE) {
				...
				if (IS_SW_NOA_TIMER(pAd) && (pP2PCtrl->GONoASchedule.Count > 100)) {
					if (CFG80211_P2pAdjustSwNoATimer(pAd, Value, SavedNextTargetTimePoint) == TRUE) {
...
static VOID CFG80211_P2pParseNoASubElmt(PRTMP_ADAPTER pAd, VOID *Msg, ULONG MsgLen,
...
VOID CFG80211_PeerP2pBeacon(PRTMP_ADAPTER pAd, PUCHAR pAddr2, MLME_QUEUE_ELEM *Elem,
...
VOID CFG80211_P2pClientSendNullFrame(VOID *pAdCB, INT PwrMgmt)
{
	...
	if (pEntry == NULL) {
		...
		ApCliRTMPSendNullFrame(pAd, RATE_6,
...
#endif
		{
			if (pApCliEntry->wdev.WepStatus == Ndis802_11Encryption3Enabled) {
				...
				RTMPAddWcidAttributeEntry(pAd, BssIdx, pKeyInfo->KeyId,
						pApCliEntry->SharedKey[pKeyInfo->KeyId].CipherAlg, NULL);
