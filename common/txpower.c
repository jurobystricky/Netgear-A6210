...
#ifdef SINGLE_SKU
static VOID GetSingleSkuDeltaPower(
	IN RTMP_ADAPTER *pAd,
...
VOID InitLookupTable(PRTMP_ADAPTER pAd)
{
...
static VOID AsicGetAutoAgcOffsetForTemperatureSensor(RTMP_ADAPTER *pAd, CHAR *pDeltaPwr,
	CHAR *pTotalDeltaPwr, CHAR *pAgcCompensate, CHAR *pDeltaPowerByBbpR1)
{
...
#if 0 //JB removed
static VOID AsicGetAutoAgcOffsetForExternalTxAlc(PRTMP_ADAPTER pAd, PCHAR pDeltaPwr,
	PCHAR pTotalDeltaPwr, PCHAR pAgcCompensate, PCHAR pDeltaPowerByBbpR1)
{
	...
	if (pAd->Mlme.OneSecPeriodicRound % 4 == 0) {
		...
		if (bAutoTxAgc) {
			...
			/* TSSI representation */
			if (IS_RT3071(pAd) || IS_RT3390(pAd) || IS_RT3090A(pAd) || IS_RT3572(pAd)) {
				/* 5-bits */
...
static VOID AsicCompensatePowerViaBBP(RTMP_ADAPTER *pAd, CHAR *pTotalDeltaPower)
{
	UCHAR mdsm_drop_pwr;
...
static VOID AsicPercentageDeltaPower(PRTMP_ADAPTER pAd, CHAR Rssi, PCHAR pDeltaPwr,
	PCHAR pDeltaPowerByBbpR1)
{
	/*
	...
	*/
	...
	/* 61 ~ 90%, treat as 75% in terms of mW		 DeltaPwr -= 1; */
		*pDeltaPwr -= 1;
		...
		*pDeltaPwr -= 3;
		...
		/* 0 ~ 9 %, treat as MIN(~3%) in terms of mW	 DeltaPwr -= 12; */
...
VOID AsicAdjustTxPower(RTMP_ADAPTER *pAd)
{
	...
	if(INFRA_ON(pAd)) {
		Rssi = RTMPMaxRssi(pAd, pAd->StaCfg.RssiSample.AvgRssi0,
				pAd->StaCfg.RssiSample.AvgRssi1, pAd->StaCfg.RssiSample.AvgRssi2);
		}
	}
#endif /* CONFIG_STA_SUPPORT */
	...
	DBGPRINT(RT_DEBUG_INFO,
			...
			__FUNCTION__, DeltaPwr, TotalDeltaPower, TxAgcCompensate, DeltaPowerByBbpR1));
	...
	for (i = 0; i < 7; i++) {
		...
		if (IS_RT5390(pAd) || IS_RT5392(pAd) || IS_RT5592(pAd) || IS_RT3290(pAd) || IS_RT65XX(pAd))
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, (
					...
					__FUNCTION__, i * 2, pAd->TxPower[i * 2].Power, i * 2 + 1,
		...
		{ /* Default routine. RT3070 and RT3370 run here. */
			...
			pAd->TxPower[i * 2 + 1].Power = Power.field.Byte1;
			if(!IS_RT3390(pAd)) {
				// 3370 has different Tx power range
