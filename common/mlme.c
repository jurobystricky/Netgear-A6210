...
static void MlmeAutoScan(RTMP_ADAPTER *pAd);
//static void MlmeCheckForRoaming(RTMP_ADAPTER *pAd, ULONG Now32);
static BOOLEAN MlmeCheckForFastRoaming(RTMP_ADAPTER *pAd);
static void BssCipherParse(BSS_ENTRY *pBss);
static BOOLEAN MlmeDequeue(MLME_QUEUE *Queue, MLME_QUEUE_ELEM **Elem);
static void MlmeRestartStateMachine(RTMP_ADAPTER *pAd);
static BOOLEAN  MlmeQueueEmpty(MLME_QUEUE *Queue);
static BOOLEAN  MlmeQueueFull(MLME_QUEUE *Queue, UCHAR SendId);
static void MlmeQueueDestroy(MLME_QUEUE *Queue);
static void AsicEvaluateRxAnt(RTMP_ADAPTER *pAd);

static void MlmeCalculateChannelQuality(RTMP_ADAPTER *pAd,
	PMAC_TABLE_ENTRY pMacEntry, ULONG Now);


#ifdef CONFIG_STA_SUPPORT
static void AsicStaBbpTuning(RTMP_ADAPTER *pAd);
#endif
...
static CHAR RssiSafeLevelForTxRate[] ={  -92, -91, -90, -87, -88, -86, -85, -83, -81, -78, -72, -71, -40, -40 };
#endif //0
...
#if 0 //JB removed
static void dynamic_ed_cca_threshold_adjust(RTMP_ADAPTER * pAd)
{
	UINT32 reg_val = 0;
...
static void update_rssi_for_channel_model(RTMP_ADAPTER * pAd)
{
...
#if 0 //JB removed
static void dynamic_cck_mrc(RTMP_ADAPTER * pAd)
{
	UINT32 bbp_val = 0;
...
#ifdef CONFIG_AP_SUPPORT
#ifdef MT76x2
static void periodic_monitor_false_cca_adjust_vga(RTMP_ADAPTER *pAd)
{
...
#ifdef CONFIG_STA_SUPPORT
static void periodic_monitor_rssi_adjust_vga(RTMP_ADAPTER *pAd)
{
	if ((pAd->CommonCfg.lna_vga_ctl.bDyncVgaEnable) &&
...
#ifdef CONFIG_STA_SUPPORT
#if 0 //JB removed
static void periodic_check_channel_smoothing(RTMP_ADAPTER *ad)
{
	UINT32 bbp_value;
	...
	while (!MlmeQueueEmpty(&pAd->Mlme.Queue)) {
		...
		if (MlmeDequeue(&pAd->Mlme.Queue, &Elem)) {
			...
			case WPA_STATE_MACHINE:
				StateMachinePerformAction(pAd, &pAd->Mlme.WpaMachine,
						Elem, pAd->Mlme.WpaMachine.CurrState);
				break;
	...
	PVOID SystemSpecific2, PVOID SystemSpecific3)
{
	...
	if (pAd->CommonCfg.MO_Cfg.bEnable) {
		...
		if (stage == MO_MEAS_PERIOD) {
			if (pAd->chipOps.AsicMeasureFalseCCA != NULL)
			...
			if (pAd->CommonCfg.MO_Cfg.nFalseCCACnt > pAd->CommonCfg.MO_Cfg.nFalseCCATh)
				if (pAd->chipOps.AsicMitigateMicrowave != NULL)
	...
	*/
	...
	if ((RTMP_TEST_FLAG(pAd, (fRTMP_ADAPTER_HALT_IN_PROGRESS |
		fRTMP_ADAPTER_RADIO_OFF | fRTMP_ADAPTER_RADIO_MEASUREMENT |
...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
	...
	if (pAd->Mlme.PeriodicRound % MLME_TASK_EXEC_MULTIPLE == 0) {
		...
		if (pAd->chipOps.SecondCCADetection != NULL)
			pAd->chipOps.SecondCCADetection(pAd);
...
void STAMlmePeriodicExec(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->chipOps.HighPowerTuning != NULL)
		pAd->chipOps.HighPowerTuning(pAd, &pAd->StaCfg.RssiSample);
	...
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) {
		...
		if (pAd->chipOps.TemperCompensation != NULL)
			pAd->chipOps.TemperCompensation(pAd);
	}
...
/* 1x1 device for 802.11n WMM Test*/
	...
	if (CQI_IS_DEAD(pAd->Mlme.ChannelQuality)) {
		...
		if ((pAd->StaCfg.bAutoReconnect == TRUE)
			...
			&& (MlmeValidateSSID(pAd->MlmeAux.AutoReconnectSsid,
...
static BOOLEAN MlmeCheckForFastRoaming(RTMP_ADAPTER *pAd)
{
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		max_rssi = RTMPMaxRssi(pAd, pAd->StaCfg.RssiSample.LastRssi0,
...
/*		(! RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))*/
		...
		if (!(pAd->StaCfg.UapsdInfo.bAPSDCapable && pAd->CommonCfg.APEdcaParm.bAPSDCapable)) {
			RTMPSendNullFrame(pAd, pAd->CommonCfg.TxRate,
					...
					fOP_STATUS_WMM_INUSED) ? TRUE:FALSE),
					pAd->CommonCfg.bAPSDForcePowerSave ? PWR_SAVE : pAd->StaCfg.Psm);
		} else {
...
/*
	...
	==========================================================================
 */
void MlmeCalculateChannelQuality(PRTMP_ADAPTER pAd, PMAC_TABLE_ENTRY pMacEntry,
	ULONG Now32)
{
	...
	*/
...
#endif /* CONFIG_STA_SUPPORT */
	{
		...
		ChannelQuality = (RSSI_WEIGHTING * NorRssi +
				TX_WEIGHTING * (100 - TxPRR) +  RX_WEIGHTING* (100 - RxPER)) / 100;
	}
...
/*
	...
	==========================================================================
*/
static void UpdateBasicRateBitmap(RTMP_ADAPTER *pAdapter)
{
	int i, j;
...
void MlmeUpdateHtTxRates(RTMP_ADAPTER *pAd, UCHAR apidx)
{
	...
	if ((ADHOC_ON(pAd) || INFRA_ON(pAd)) && (pAd->OpMode == OPMODE_STA)) {
		...
		BasicMCS =pAd->MlmeAux.AddHtInfo.MCSSet[0]+
				(pAd->MlmeAux.AddHtInfo.MCSSet[1]<<8)+(StbcMcs<<16);
		if ((pAd->CommonCfg.DesiredHtPhy.TxSTBC) &&
			(pRtHtCap->RxSTBC) && (pAd->Antenna.field.TxPath == 2)) {
...
#endif /* CONFIG_STA_SUPPORT */
	{
		...
		BasicMCS = pAd->CommonCfg.AddHTInfo.MCSSet[0]+
	...
	DBGPRINT(RT_DEBUG_TRACE,
			...
			pHtPhy->field.BW, pHtPhy->field.ShortGI,
			pHtPhy->field.MODE));
	DBGPRINT(RT_DEBUG_TRACE,("%s():<=== \n", __FUNCTION__));
}
		...
		if ((Tab->BssEntry[i].Channel == Channel) &&
			...
			for (j = i; j < Tab->BssNr - 1; j++) {
				...
				NdisMoveMemory(&(Tab->BssEntry[j]),
						&(Tab->BssEntry[j + 1]), sizeof(BSS_ENTRY));
				if (pOldAddr) {
					RTMPZeroMemory(pOldAddr, MAX_VIE_LEN);
...
void BssEntrySet(RTMP_ADAPTER *pAd, BSS_ENTRY *pBss, BCN_IE_LIST *ie_list,
	CHAR Rssi, USHORT LengthVIE, PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	if (ie_list->HtCapabilityLen> 0) {
		...
		if (ie_list->AddHtInfoLen > 0) {
			...
			pBss->CentralChannel = get_cent_ch_by_htinfo(pAd,
					&ie_list->AddHtInfo,
 ...
 */
ULONG BssTableSetEntry(PRTMP_ADAPTER pAd, BSS_TABLE *Tab, BCN_IE_LIST *ie_list,
	CHAR Rssi, USHORT LengthVIE, PNDIS_802_11_VARIABLE_IEs pVIE)
{
	ULONG Idx;
#ifdef APCLI_SUPPORT
...
//JB Why is this not void?
int TriEventTableSetEntry(RTMP_ADAPTER *pAd, TRIGGER_EVENT_TAB *Tab,
	UCHAR *pBssid, HT_CAPABILITY_IE *pHtCapability, UCHAR HtCapabilityLen,
 ...
 */
BOOLEAN MlmeEnqueue(RTMP_ADAPTER *pAd, ULONG Machine, ULONG MsgType, ULONG MsgLen,
	void *Msg, ULONG Priv)
{
	int Tail;
 ...
 *	\post
 */
BOOLEAN MlmeEnqueueForRecv(RTMP_ADAPTER *pAd, ULONG Wcid, ULONG TimeStampHigh,
	ULONG TimeStampLow, UCHAR Rssi0, UCHAR Rssi1, UCHAR Rssi2, ULONG MsgLen,
	void *Msg, UCHAR Signal, UCHAR OpMode)
{
...
#endif /* P2P_SUPPORT || RT_CFG80211_P2P_SUPPORT */
	{
		...
		*/
		do {
			...
			*/
			for (i = 0; i < pAd->ApCfg.ApCliNum; i++) {
				if (MAC_ADDR_EQUAL(pAd->ApCfg.ApCliTab[i].MlmeAux.Bssid,
					pFrame->Hdr.Addr2)) {
					bToApCli = TRUE;
					break;
				}
 ...
 */
static void MlmeQueueDestroy(MLME_QUEUE *pQueue)
{
 ...
 */
#ifdef CONFIG_STA_SUPPORT
BOOLEAN MsgTypeSubst(RTMP_ADAPTER *pAd, FRAME_802_11 *pFrame, int *Machine,
	int *MsgType)
{
	USHORT Seq, Alg;
	UCHAR EAPType;
 ...
 */
void StateMachineInit(STATE_MACHINE *S, STATE_MACHINE_FUNC Trans[], ULONG StNr,
	ULONG MsgNr, STATE_MACHINE_FUNC DefFunc, ULONG InitState, ULONG Base)
{
	ULONG i, j;
 ...
 *	\return   None
 */
void StateMachinePerformAction(PRTMP_ADAPTER pAd, STATE_MACHINE *S,
...
/*
	...
	Return Value:
		FALSE 	if pAd->CommonCfg.SupportedHtPhy doesn't accept
			the pHtCapability.  (AP Mode)
...
/*
	...
	========================================================================
*/
BOOLEAN RTMPCheckEntryEnableAutoRateSwitch(RTMP_ADAPTER *pAd,
	MAC_TABLE_ENTRY *pEntry)
{
	BOOLEAN result = TRUE;
...
#if 0 //JB removed
static BOOLEAN RTMPAutoRateSwitchCheck(RTMP_ADAPTER *pAd)
{
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		int apidx = 0;
...
static void AsicStaBbpTuning(RTMP_ADAPTER *pAd)
{
	...
	if ((pAd->OpMode == OPMODE_STA)
		...
		if (pAd->chipOps.ChipAGCAdjust != NULL)
			R66 = pAd->chipOps.ChipAGCAdjust(pAd, Rssi, R66);
