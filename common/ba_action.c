...
static BUILD_TIMER_FUNCTION(BARecSessionIdleTimeout);
...
{
	PNDIS_PACKET pPacket = mpdu->pPacket;
...
static USHORT ba_indicate_reordering_mpdus_in_order(PRTMP_ADAPTER pAd,
	PBA_REC_ENTRY pBAEntry, USHORT StartSeq)
{
	struct reordering_mpdu *mpdu_blk;
...
static void ba_indicate_reordering_mpdus_le_seq(PRTMP_ADAPTER pAd,
	PBA_REC_ENTRY pBAEntry, USHORT Sequence)
{
	struct reordering_mpdu *mpdu_blk;
...
static BOOLEAN BARecSessionAdd(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry,
...
//	NDIS_STATUS NStatus;
	...
	if (pMacEntry) {
		...
		if (ADHOC_ON(pAd)) {
			ActHeaderInit(pAd, &ADDframe.Hdr, pAddr,
					pAd->StaCfg.wdev.if_addr, pAd->CommonCfg.Bssid);
		} else
#endif /* CONFIG_STA_SUPPORT */
		{
			ActHeaderInit(pAd, &ADDframe.Hdr, pAddr,
					pMacEntry->wdev->if_addr, pMacEntry->wdev->bssid);
		}
	}
#else
#ifdef CONFIG_AP_SUPPORT
	...
	ADDframe.BaParm.BufSize = min(((UCHAR)pAddreqFrame->BaParm.BufSize),
...
BOOLEAN CntlEnqueueForRecv(RTMP_ADAPTER *pAd, ULONG Wcid, ULONG MsgLen,
	PFRAME_BA_REQ pMsg)
{
...
static void convert_reordering_packet_to_preAMSDU_or_802_3_packet(PRTMP_ADAPTER pAd,
...
*/
	...
	RTMP_OS_PKT_INIT(pRxBlk->pRxPacket, get_netdev_from_bssid(pAd, FromWhichBSSID),
	...
	if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			RT_VLAN_8023_HEADER_COPY(pAd, VLAN_VID, VLAN_Priority,
					Header802_3, LENGTH_802_3, data_p,
					FromWhichBSSID, TPID);
		}
#endif /* CONFIG_AP_SUPPORT */
	...
	RX_BLK *pRxBlk, UCHAR FromWhichBSSID)
{
	...
	if ((mpdu_blk != NULL) &&
		...
		*/
		...
		DBGPRINT(RT_DEBUG_ERROR,
				("!!! %s: (%d) Can't allocate reordering mpdu blk\n",
...
void Indicate_AMPDU_Packet(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, UCHAR FromWhichBSSID)
{
	...
	if (SEQ_STEPONE(Sequence, pBAEntry->LastIndSeq, MAXSEQ)) {
		...
		pBAEntry->LastIndSeq = Sequence;
		INDICATE_LEGACY_OR_AMSDU(pAd, pRxBlk, FromWhichBSSID);
 		LastIndSeq = ba_indicate_reordering_mpdus_in_order(pAd,
				pBAEntry, pBAEntry->LastIndSeq);
