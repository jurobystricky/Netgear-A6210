...
#if 0 //JB removed
static BOOLEAN eFuseWriteRegisters(
	IN	PRTMP_ADAPTER	pAd,
	IN	USHORT Offset,
	IN	USHORT Length,
	IN	USHORT* pData);
...
========================================================================
*/
UCHAR eFuseReadRegisters(PRTMP_ADAPTER pAd, USHORT Offset, USHORT Length, USHORT* pData)
{
...
int rtmp_ee_efuse_read16(RTMP_ADAPTER *pAd, USHORT Offset, USHORT *pValue)
{
...
int rtmp_ee_efuse_write16(RTMP_ADAPTER *pAd, USHORT Offset, USHORT data)
{
...
INT rtmp_ee_write_to_efuse(PRTMP_ADAPTER pAd)
{
...
VOID eFuseGetFreeBlockCount(PRTMP_ADAPTER pAd, PUINT EfuseFreeBlock)
{
	...
	*EfuseFreeBlock = 0;
	...
	/*if not find last free block, return free block number = 0, this should not
	 * happen since we have checked first free block number previously*/
...
BOOLEAN eFuse_init(RTMP_ADAPTER *pAd)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("NVM is Efuse and its size =%x[%x-%x] \n",
			pAd->chipCap.EFUSE_USAGE_MAP_SIZE,pAd->chipCap.EFUSE_USAGE_MAP_START,
...
BOOLEAN set_BinModeWriteBack_Proc(PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT bEnable = simple_strtol(arg, 0, 10);
