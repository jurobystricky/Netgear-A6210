 ...
 */
//static UCHAR BaSizeArray[4] = {8,16,32,64};
...
void BuildChannelList(RTMP_ADAPTER *pAd)
{
	...
	if (!WMODE_5G_ONLY(pAd->CommonCfg.PhyMode)) {
		...
		if (num > 0) {
			pChannelList = os_alloc_mem(num * sizeof(UCHAR));
			if (!pChannelList) {
				return;
			}

			pChannelListFlag = os_alloc_mem(num * sizeof(UCHAR));
			if (!pChannelListFlag) {
	...
	if (WMODE_CAP_5G(pAd->CommonCfg.PhyMode)) {
		...
		if (num > 0) {
			UCHAR RadarCh[15] = {52, 56, 60, 64, 100, 104, 108, 112,
			...
			for (i = 0; i < num; i++) {
				if ((pAd->CommonCfg.bIEEE80211H == 0)||
					...
					/*
					 * Based on the requiremnt of FCC, some channels
					 * could not be used anymore when test DFS function.
					 */
	...
	DBGPRINT(RT_DEBUG_TRACE,
			...
			pAd->CommonCfg.CountryRegion, pAd->CommonCfg.CountryRegionForABand,
			pAd->RfIcType, pAd->CommonCfg.PhyMode, pAd->ChannelListNum));

#ifdef RT_CFG80211_SUPPORT
	for (i = 0; i < pAd->ChannelListNum;i++) {
		CFG80211OS_ChanInfoInit(pAd->pCfg80211_CB, i,
				pAd->ChannelList[i].Channel, pAd->ChannelList[i].MaxTxPwr,
...
UCHAR NextChannel(RTMP_ADAPTER *pAd, UCHAR channel)
{
	...
	for (i = 0; i < (pAd->ChannelListNum - 1); i++) {
		if (channel == pAd->ChannelList[i].Channel) {
			...
			/* 2009 PF#2: Need to handle the second channel of AP fall 
			 * into affected channel range.*/
 ...
 */
#if 0 //JB removed
static UCHAR RTMPFindScanChannel(PRTMP_ADAPTER pAd, UINT8 LastScanChannel)
{
...
CHAR ConvertToRssi(RTMP_ADAPTER *pAd, CHAR Rssi, UCHAR rssi_idx)
{
	...
	/*
		...
		SW/QA owners should read the "external-LNA gain" and "RSSI OFFSET"
		content in EEPROM as "SIGNED".
...
static void Handle_BSS_Width_Trigger_Events(RTMP_ADAPTER *pAd)
{
	ULONG Now32;
