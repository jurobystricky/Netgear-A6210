...
static PCH_REGION GetChRegion(PUCHAR CntryCode)
{
	int loop = 0;
...
static void ChBandCheck(UCHAR PhyMode, PUCHAR pChType)
{
	*pChType = 0;
...
static UCHAR FillChList(PRTMP_ADAPTER pAd, PCH_DESP pChDesp, UCHAR Offset,
	UCHAR increment, UCHAR regulatoryDomain)
{
	int i, j, l;
	UCHAR channel;
...
static inline void CreateChList(PRTMP_ADAPTER pAd, PCH_REGION pChRegion,
	IN UCHAR Geography)
{
	int i;
	UCHAR offset = 0;
	...
	for (i = 0; pChDesp[i].FirstChannel != 0; i++) {
		...
		if ((pChDesp[i].Geography == BOTH) || (Geography == BOTH) ||
...
void BuildChannelListEx(PRTMP_ADAPTER pAd)
{
	PCH_REGION pChReg;
...
static PCOUNTRY_PROP GetCountryProp(PUCHAR CntryCode)
{
	int loop = 0;
	PCOUNTRY_PROP pCountryProp = NULL;

	while (strcmp((PSTRING) CountryProp[loop].CountReg, "") != 0) {
		if (strncmp((PSTRING) CountryProp[loop].CountReg,
			(PSTRING) CntryCode, 2) == 0) {
...
static BOOLEAN GetEDCCASupport(PRTMP_ADAPTER pAd)
{
	BOOLEAN ret = FALSE;
...
#ifdef DOT11_N_SUPPORT
static BOOLEAN IsValidChannel(PRTMP_ADAPTER pAd, UCHAR channel)
...
static UCHAR GetExtCh(UCHAR Channel, UCHAR Direction)
{
	CHAR ExtCh;
...
BOOLEAN N_ChannelGroupCheck(PRTMP_ADAPTER pAd, UCHAR Channel)
{
...
UINT8 GetCountryMaxTxPwr(PRTMP_ADAPTER pAd, UINT8 channel)
{
	int i;
	for (i = 0; i < pAd->ChannelListNum; i++) {
		if (pAd->ChannelList[i].Channel == channel)
...
/* for OS_ABL */
void RTMP_MapChannelID2KHZ(UCHAR Ch, UINT32 *pFreq)
{
	int chIdx;
...
/* for OS_ABL */
void RTMP_MapKHZ2ChannelID(ULONG Freq, int *pCh)
{
	int chIdx;
	for (chIdx = 0; chIdx < CH_HZ_ID_MAP_NUM; chIdx++) {
